...
// NewServer creates a new api turbo octo eureka server but does not configure it
func NewServer(api *operations.TurboOctoEurekaAPI) *Server {
	s := new(Server)
	s.api = api
	return s
}
...
// Server for the turbo octo eureka API
type Server struct {
	EnabledListeners []string `long:"scheme" description:"the listeners to enable, this can be repeated and defaults to the schemes in the swagger spec"`

	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/turbo-octo-eureka.sock"`
	...
	api          *operations.TurboOctoEurekaAPI
...
func (s *Server) SetAPI(api *operations.TurboOctoEurekaAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(1)
		s.Logf("Serving turbo octo eureka at unix://%s", s.SocketPath)
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(1)
		s.Logf("Serving turbo octo eureka at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(1)
		s.Logf("Serving turbo octo eureka at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
